...
try:
    ...
    # IMPORTANT:
    # Do NOT extract article pages from arbitrary PDF text.
    # A value such as "С. 1" can be the first page of the
    # whole PDF, not the article itself.
    result["pages"] = ""
...
# Crossref is queried ONLY by the exact DOI already
# found in the source. We never search Crossref by title.
...
# Do not replace the original article title with
# Crossref title. The source itself determines identity.
if (
    not item.get("title")
...
# IMPORTANT:
# For exact DOI, Crossref is authoritative for
# volume / issue / pages. It can correct bad metadata
# extracted from a webpage.
if crossref.get("volume"):
    item["volume"] = crossref["volume"]

if crossref.get("issue"):
    item["issue"] = crossref["issue"]

if crossref.get("pages"):
    item["pages"] = crossref["pages"]
...
# Normalize DOI before sending it to frontend.
if doi:
    doi = (
        f"https://doi.org/"
        f"{normalize_doi(doi)}"
